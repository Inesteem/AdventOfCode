...
//        if(!reg_mem[words[REG_DST]]) return 1;
        if(tasks.empty()) return 0;
        reg_mem[words[REG_DST]] = tasks.front();
        tasks.pop();
 
    } else if(words[INSTR] == "jgz"){
        bool is_cond_digit = (words[REG_DST].find_first_not_of( "0123456789-" ) == string::npos);
        if(is_cond_digit){ 
            if(stoi(words[REG_DST]) <= 0) return 1;
        } else if(reg_mem[words[REG_DST]] <= 0) return 1;
        int ret;
        if(is_digit)
...
int main(int argc, char *argv[]){
   ...
   //         print(lines[pos2]);
     ...
     //       print_map(2);
        ...
        if((pos1 < 0 || pos1 >= lines.size()) && (pos2 < 0 || pos2 >= lines.size())) break;
    }
    cout << "prog 1 send " << snt_count << " times a value!" << endl;
	return 0;
}
